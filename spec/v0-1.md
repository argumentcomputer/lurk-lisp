...
When `<c>` evaluates to a character, the expression `(num <c>)` evaluates to the `u32` ASCII value of the character `<c>`. 

When `<n>` evaluates to a Num, the expression `(char <n>)` evaluates to the character which has the ASCII value of `<n>`. If `<n>` is not a valid ASCII value, an error is thrown.

It is an error to call `num` on anything not a character and to call `char` on anything not a Num.

While `num` and `char` is part of `lurk-rs`, it is not currently supported by core Lurk.
...
The expression `(= <e1> <e2>)` evaluates by first evaluating the expressions `<e1>` and `<e2>`. The resulting values should be Nums. Then, the numers are compared for equality (over the prime field).
...
The release specs of `lurk-rs` has diverged from core Lurk. The following parsing rules are only supported by `lurk-rs`. 

The `lurk-rs` reader has support for parsing negative Nums `-<n>` and fractional Nums `<n>/<m>`. 
...
Note: the parser for these has only one character of lookahead and hence are whitespace sensitive.

## Values

The values of the language are `t`, `nil`, procedures such as those created by `lambda`, (field) Nums such as `18`, symbols such as the `foo` datum resulting from the evaluation of the quoted expression `'foo`, strings such as `"foo"`, characters such as `#\a`, and cons cells such as `(1 . 2)` or `(1 a "t")`.
