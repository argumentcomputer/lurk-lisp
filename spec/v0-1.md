...
When `<e>` evaluates to a Num, the expression `(comm <e>)` returns a Lurk commitment primitive. 

The expression `(commit <e>)` evaluates `<e>` and commits the result, returning the commitment primitive associated with `<e>`. Lurk allows commitments to _any_ Lurk expression. 
...
The expression `(open <e>)` evaluates `<e>` and, if the result is either an commitment primitive or a field element, opens the commitment and returns its value. 
...
The values of the language are `t`, `nil`, procedures such as those created by `lambda`, (field) Nums such as `18`, symbols such as the `foo` datum resulting from the evaluation of the quoted expression `'foo`, strings such as `"foo"`, characters such as `#\a`, conses such as `(1 . 2)` or `(1 a "t")`, and commitments such as `(comm 0x185e06...c7aa7f2)`.
