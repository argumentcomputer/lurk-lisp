...
When `<c>` evaluates to a character, the expression `(num <c>)` evaluates to the `u32` ASCII value of the character `<c>`. When `<n>` evaluates to a number, `(num <n>)` evaluates to `<n>`.
